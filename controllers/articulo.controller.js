...
exports.get_articulos = utils.wrapAsync(async function (req, res, next) {
  try {
    ...
    await Articulo.get_articulos()
      .then(
        (articulo) => res.status(200).json(articulo),
      ...
      try {
        await Articulo.add_articulo(articulo)
          .then((articulo) => {
            ...
            logger.access.info(
              utilsLogs.creadoCorrectamente("articulo", articulo._id)
            );
          })
          .catch((err) => {
            res.status(406).json(utils.parametrosIncorrectos());
            logger.warning.warn(utilsLogs.parametrosIncorrectos());
            console.log(
              "El error está dentro de la función Articulo.add_articulo"
            );
          });
      } catch (err) {
        res.status(406).json(utils.parametrosIncorrectos());
        ...
        await Articulo.edit_articulo(id, articulo)
          .then((result) => {
            if (result.value === null) {
              ...
              logger.access.info(
                utilsLogs.creadoCorrectamente("articulo", result.value._id)
              );
            }
          })
          .catch((err) => {
      ...
      await Articulo.delete_articulo(id)
        .then((info) => {
          if (info === null) {
            ...
            logger.access.info(
              utilsLogs.borradoCorrectamente("articulo", info._id)
            );
          }
        })
        .catch((err) => {
