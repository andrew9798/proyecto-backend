...
exports.get_usuario_by_correo_and_password = async function (req, res, next) {
  ...
  try {
    try {
      await Usuario.findByemail(
        emailycontraseña.Correo,
...
exports.add_usuario = utils.wrapAsync(async function (req, res, next) {
  ...
  if (
    ...
    try {
      await dbConn.conectar;
      try {
        await Usuario.add_usuario(usuario)
          .then((result) => {
            res.status(201).json(utils.creadoCorrectamente("usuario"));
            logger.access.info(
              utilsLogs.creadoCorrectamente("usuario", result._id)
            );
          })
          .catch((err) => {
            res.status(406).json(utils.parametrosIncorrectos());
 ...
 * @param {JSON Object} res
 */
exports.edit_usuario = utils.wrapAsync(async function (req, res, next) {
  const id = req.params.id;
  const usuario = req.body;
        ...
        await Usuario.edit_usuario(id, usuario)
          .then((result) => {
            if (result.value === null) {
              ...
              logger.access.info(
                utilsLogs.creadoCorrectamente("usuario", result.value._id)
              );
            }
          })
          .catch((err) => {
  ...
  try {
    ...
    try {
      await Usuario.delete_usuario(id)
        .then((info) => {
          if (info === null) {
            ...
            logger.access.info(
              utilsLogs.borradoCorrectamente("usuario", info._id)
            );
          }
        })
        .catch((err) => {
      ...
      if (
        ...
        logger.warning.warn(utilsLogs.parametrosIncorrectos());
        throw new ParametrosIncorrectosError(utils.parametrosIncorrectos());      } else {
        throw err;
      }
    }
  } catch (err) {
